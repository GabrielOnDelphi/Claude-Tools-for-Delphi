---
name: light-Align
description: Drive a relentless one-question-at-a-time design interview before any non-trivial Delphi task — a feature, a refactor, a bug investigation, or a design decision. Each question carries your own recommended answer; you skip everything you can answer yourself (recorded in an auto-decided list) and ask the highest-stakes Delphi questions first. Reaches a shared design concept BEFORE any plan or code, so the AI cannot jump straight to a solution. Adaptive: ~5 questions for a tiny task, up to ~40 for a major one. Use when the user says "align first", "interview me", "ask me questions before you build", or starts any non-trivial work.
---

# /light-Align — Align on the design before building
...
This is the standalone interview, usable before ANY non-trivial task. `light-code-NewFeature` Phase 1 is the same interview inside the feature flow (it then continues to PRD → slices → implement); it should reference this skill rather than restating it. **No beep** — this skill ends by handing a question back to the user, who is present.
