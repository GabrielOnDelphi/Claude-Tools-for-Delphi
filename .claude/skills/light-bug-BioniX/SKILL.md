...
1. Read `~\.claude\skills\light-bug-MadShi\SKILL.md` and follow it exactly, with the product fixed to **BioniX** (the `[BioniX]` block in `products.ini`). Skip its "resolve the product" question — the product is already known.
