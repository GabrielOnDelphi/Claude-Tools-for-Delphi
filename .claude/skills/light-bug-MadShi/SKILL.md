...
Do this in order, whichever of the three routes reached this skill: Gabriel typed the name; the general `light-bug` skill saw a `.mad` file or the words "crash report" and routed here at its Step 0; or `light-bug-BioniX` came here with the product already fixed to BioniX. Be terse. Each step is a single tool call unless noted.

The pipeline is product-driven. Every product is one block in `c:\Users\<you>\.claude\skills\light-bug-MadShi\products.ini` (mbox list + source paths + fix-log location + memory subfolder). Read that file first — it is the single source of routing facts.
...
& 'c:\Users\<you>\.claude\skills\light-bug-MadShi\Tools\extract-mad.ps1' -MboxPath $mboxes -List -OutPrefix '<OutPrefix>'
```
...
```powershell
& 'c:\Users\<you>\.claude\skills\light-bug-MadShi\Tools\extract-mad.ps1' -MboxPath $mboxes -Index <N> -OutPrefix '<OutPrefix>'
```
