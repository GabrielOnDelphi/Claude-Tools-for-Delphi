...
description: Run the full three-stage Delphi code review pipeline on a file, a folder, or a set of files. Stage 1 finds correctness bugs, stage 2 counter-analyzes the findings and applies the fixes that survive, stage 3 verifies those fixes and compiles. Use when the user invokes `/light-code-Review`, says "review this Delphi code", "code review FormFoo.pas", "deep review the Lib folder", or asks for a thorough review of project `.pas` files.
---
