---
name: light-review
description: Run the full three-stage Delphi code review pipeline on a file, a folder, or a set of files. Stage 1 finds correctness bugs, stage 2 counter-analyzes the findings and applies the fixes that survive, stage 3 verifies those fixes and compiles. Use when the user invokes `/light-review`, says "review this Delphi code", "code review FormFoo.pas", "deep review the Lib folder", or asks for a thorough review of project `.pas` files.
---
...
describing the old code, so it runs the `light-update-md` skill to catch that drift — it
...
2. The **complete stage-2 report** you captured — paste it under `--- STAGE 2 REPORT ---`.
   ...
   verify the listed edits and will NOT beep (this skill beeps instead, in Step 7).
...
## Step 5 — Keep the docs in sync (`light-update-md`)
...
Otherwise, invoke the **`light-update-md`** skill (via the **Skill** tool). In the same
...
`light-update-md` already verifies every claim against the code and edits only on real drift,
...
## Step 7 — Consolidated summary + beep
...
Then beep once so the AFK user knows the pipeline finished:
...
- **One beep, at the very end.** The agents do not beep — only this skill does, once, in
  Step 7, after the review-log step.
