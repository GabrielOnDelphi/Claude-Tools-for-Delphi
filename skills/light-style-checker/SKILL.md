...
description: Scan imported or 3rd-party Delphi code for style compliance, common mistakes, and dangerous patterns (resource leaks, unsafe casts, missing try/finally). Launches the light-style-checker agent. Use when the user invokes `/light-style-checker`, says "check this 3rd-party unit", "scan the imported library", or "style-check SourceCode/". Do NOT use for our own project code — use /light-review for that.
---

# /light-style-checker — 3rd-party Code Style Audit
...
project code, the right tool is `/light-review` (deeper, real correctness review). If the user
points this at our own code, say so and suggest `/light-review` — then proceed only if they
confirm.
...
- **3rd-party scope only.** If the target looks like our own project code, recommend `/light-review` instead before proceeding.
